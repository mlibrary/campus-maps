...
class RoleForceForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('force_password_change')) {
      ...
      if ($this->configFactory->get('force_password_change.settings')->get('check_login_only')) {
        \Drupal::messenger()->addMessage($this->t('Users in this role will be required to change their password on next login'));
      }
      else {
        \Drupal::messenger()->addMessage($this->t('Users in this role will be required to immediately change their password'));
      }
    }
  }
