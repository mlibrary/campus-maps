...
class ForcePasswordChangeEventSubscriber implements EventSubscriberInterface {
   ...
   *
   * If either of the two conditions above returns true,
   * they are redirected to the change password page
   */
  public function checkForPasswordForce($event) {
    // Only do something if the module is set to be enabled.
    // This can only be changed in settings.php, and is added
    // to allow users to disable the module if a problem arises.
    if ($this->configFactory->get('force_password_change.settings')->get('enabled')) {
      // Check whether force password testing is done on every page, or
      // only on login. We only need to do the following code if the user
      // is logged in, and we are checking on every page load.
      if ($this->currentUser->id() && !$this->configFactory->get('force_password_change.settings')->get('check_login_only')) {
        // We should check for AJAX, but the request isn't. Sniffing headers
        // isn't a perfect method, but this header should be
         ...
         */
        $authorizedPaths = [
          Url::fromRoute('entity.user.edit_form', ['user' => $this->currentUser->id()])->toString(),
          Url::fromRoute('user.logout')->toString(),
          Url::fromRoute('user.pass')->toString(),
        ];
        if (!$ajax_request && !in_array($this->currentPath->getPath(), $authorizedPaths)) {
        ...
        if ($redirect) {
          // The user is redirected. A message needs to be set informing them of
          // the reason for being redirected to the password change page.
          if ($redirect == 'admin_forced') {
            \Drupal::messenger()->addMessage(t('An administrator has required that you change your password. Please change your password to proceed.'), 'warning', FALSE);
          }
          else {
